...
//           </div>
//           {showHandlebar && (
//             <div className="h-8 w-8 sm:h-12 sm:w-12 rounded-md sm:rounded-xl top-1/2 -translate-y-1/2 -translate-x-1/2 bg-white z-30 absolute flex items-center justify-center shadow-[0px_-1px_0px_0px_#FFFFFF40] hover:bg-gray-200 active:gap-2 active:bg-gray-200">
...
export const Compare = ({
  ...
  // **Fixed View Animation Function**
  const startViewAnimation = useCallback(() => {
    if (isViewAnimating || hasAnimated) return;

    console.log("Starting view animation"); // Debug log
    ...
    const animate = (currentTime: number) => {
      ...
      if (progress < 1) {
        ...
        setHasAnimated(true);
        setSliderXPercent(endValue); // Ensure it ends exactly at target
      }
    };
  ...
  // **Regular autoplay (disabled during view animation)**
  const startAutoplay = useCallback(() => {
